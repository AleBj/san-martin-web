  ...
  <header>
    ...
    <nav v-if="isOpen">
      ...
      <div :class="$style.navIcons" @click="close">
        ...
        <Button v-if="!isMobile" :cta="`Trámites`" :link="`/tramites`" :class="$style.button" />        
      </div>
    </nav>
    
    <Button v-if="isMobile" :cta="`Trámites`" :link="`/tramites`" :class="[$style.button, $style.tramitesMobile]" />    
